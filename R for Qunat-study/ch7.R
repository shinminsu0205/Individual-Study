library(stringr)
library(xts)
library(magrittr)

KOR_ticker = read.csv('data/KOR_ticker.csv',row.names = 1)
KOR_ticker$종목코드 = str_pad(KOR_ticker$종목코드,6,side=c('left'),pad = '0')
price_list = list()
for (i in 1:nrow(KOR_ticker)){
  name = KOR_ticker[i,'종목코드']
  price_list[[i]] =
    read.csv(paste0('data/KOR_price/',name,'_price.csv'),row.names=1)%>%
    as.xts()
}

price_list = do.call(cbind,price_list)%>%na.locf() #결측치에는 전일 데이터 사용
colnames(price_list) = KOR_ticker$종목코드
write.csv(data.frame(price_list),'data/KOR_price.csv',fileEncoding = 'EUC-KR')
#xts 파일을 그대로 csv 파일로 써버리면 인덱스가 삭제되므로 dataframe 형태로 저장

#재무제표 정리
library(stringr)
library(magrittr)
library(dplyr)

KOR_ticker = read.csv('data/KOR_ticker.csv',row.names=1)
KOR_ticker$종목코드 = 
  str_pad(KOR_ticker$'종목코드',6,side = c('left'),pad='0')
data_fs = list()

for (i in 1:nrow(KOR_ticker)){
  name = KOR_ticker[i,'종목코드']
  data_fs[[i]] = read.csv(paste0('data/KOR_fs/',name,'_fs.csv'),row.names=1)
}
fs_item = data_fs[[1]] %>% rownames()
length(fs_item)

select_fs = lapply(data_fs, function(x){
  #해당 항목이 있을 시 데이터를 선택
  if ('매출액' %in% rownames(x)){
    x[which(rownames(x)=='매출액'),] #which():()안의 조건에 맞는 원소의 위치를 반환
  }else{
    data.frame(NA)
  }
})
select_fs = bind_rows(select_fs)#rbind와 달리 열의 개수가 달라도 합쳐짐(부족한 열에는 NA가 자동으로 들어감)

#NA., . 제거
select_fs = select_fs[!colnames(select_fs) %in%
                        c('.','NA.')]
select_fs = select_fs[,order(names(select_fs))] #order:열 이름의 연도별 순서를 구한후 정렬
rownames(select_fs) = KOR_ticker[,'종목코드'] #행이름을 티커로 설정

#전체 재무항목 받아오기
fs_list = list()
for (i in 1:length(fs_item)){
  select_fs = lapply(data_fs,function(x){
    if (fs_item[i] %in% rownames(x)){
      x[which(rownames(x)==fs_item[i]),]
    }else{
      data.frame(NA)
    }
  })
  
  #리스트 데이터를 행으로 묶어줌
  select_fs = bind_rows(select_fs)
  
  #열이름이 '.' 혹은 'NA.'인 지점은 삭제
  select_fs = select_fs[!colnames(select_fs) %in%
                          c('.','NA.')]
  
  #연도 순별로 정리
  select_fs = select_fs[,order(names(select_fs))]
  
  #행 이름을 티커로 변경
  rownames(select_fs) = KOR_ticker[,'종목코드']
  
  #리스트에 최종 저장
  fs_list[[i]] = select_fs
}
#리스트 이름을 재무 항목으로 변경
names(fs_list) = fs_item
saveRDS(fs_list,'data/KOR_fs.Rds') #리스트 형태 그대로 저장(saveRDS)

##가치지표 정리하기
library(stringr)
library(magrittr)
library(dplyr)

KOR_ticker = read.csv('data/KOR_ticker.csv',row.names=1)
KOR_ticker$'종목코드' = 
  str_pad(KOR_ticker$'종목코드',6,side = c('left'),pad = '0')
data_value = list()

for(i in 1 : nrow(KOR_ticker)){
  name = KOR_ticker[i,'종목코드']
  data_value[[i]]=
    read.csv(paste0('data/KOR_value/',name,'_value.csv'),row.names=1)%>%
    t()%>%data.frame() ##csv내의 데이터가 행 형태로 돼있으므로 t()(transpose)로 열 형태로 바꿈.
}
data_value= bind_rows(data_value)
data_value = data_value[colnames(data_value)%in% c('PER','PBR','PCR','PSR')]
rownames(data_value) = KOR_ticker[,'종목코드']

data_value = data_value %>%
  mutate_all(list(~na_if(.,Inf))) #일부종목은 재무데이터가 0으로 표기돼 가치지표가 INF로 계산되는 경우가 있음.
#mutate_all()내에 na_if()함수를 이용해 Inf 데이터를 na로 변경.
write.csv(data_value,'data/KOR_value.csv',fileEncoding = 'EUC-KR')